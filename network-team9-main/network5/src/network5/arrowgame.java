...
class GamePanel extends JPanel{
    ...
    GamePanel(){
        ...
        ImageIcon img = new ImageIcon("우주선.png");
        target = new JLabel(img);
        //이미지 크기만큼 레이블 크기 설정
        target.setSize(img.getIconWidth(),img.getIconHeight());
        ImageIcon arrow = new ImageIcon("화살.png");
        ...
        public void run(){
            while(true){
                if(hit()){//타겟이 맞았다면
                    ...
                    int x=bullet.getX();
                    int y=bullet.getY()-9;//9픽셀씩 위로 이동한다.=총알 속도가 5픽셀
                    //총알이 프레임 밖으로 나갔을 때
                    if(y<0){
                        //총알 원래 위치로 이동
                        bullet.setLocation(bullet.getParent().getWidth()/2-5, bullet.getParent().getHeight()-120);
